...
namespace StackOverflowProject.Repositories
{
    ...
    public class AnswersRepository : IAnswersRepository
    {
        ...
        {
            Answer ans = db.Anwers.Where(temp => temp.AnswerID == a.AnswerID).FirstOrDefault();
            if (ans != null)
            {
        ...
        {
            Answer ans = db.Anwers.Where(temp => temp.AnswerID == aid).FirstOrDefault();
            if (ans != null)
            {
        ...
        public void DeteleteAnswer(int aid)
        {
            Answer ans = db.Anwers.Where(temp => temp.AnswerID == aid).First();
            if (ans != null)
            {
                db.Anwers.Remove(ans);
                db.SaveChanges();
        ...
        {
            List<Answer> ans = db.Anwers.Where(temp => temp.QuestionID == qid).OrderByDescending(temp => temp.AnswerDateAndTime).ToList();
            return ans;
        }

        public List<Answer> GetAnswersByAnswerID(int aid)
        {
            List<Answer> ans = db.Anwers.Where(temp => temp.AnswerID == aid).ToList();
            return ans;
        }   
    }
}
